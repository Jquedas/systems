...
function TableInfo() {
  ...
  if ($.fn.DataTable.isDataTable('#tableIntervencao2')) {
    $('#tableIntervencao2').DataTable().destroy();
  }
  ...
  veiculos.forEach((veiculo, veiculoIndex) => {
      veiculo.intervencao.forEach((interv, intervIndex) => {
        txt += "<tr>";
        txt += `<th scope='row'>${intervIndex + 1}</th>`;
        txt += `<td>${interv.refInterve}</td>`;
        txt += `<td>${interv.funcionario}</td>`;
        txt += `<td>${veiculo._matricula}</td>`;
        txt += `<td><button class='btn btn-primary' onclick='ver(${veiculoIndex}, ${intervIndex})'>Ver</button></td>`;
        txt += "</tr>";
      });
    })
    $('#tableIntervencao2 tbody').html(txt);
    $('#tableIntervencao2').DataTable();
};

function ver(veiculoIndex, intervIndex) {
  const veiculo = veiculos[veiculoIndex];
  const interv = veiculo.intervencao[intervIndex];

  // Preencher os campos do modal
  $('#infoRef').text(interv.refInterve);
  $('#infoFuncionario').text(interv.funcionario);
  $('#infoMatricula').text(veiculo._matricula);
  $('#infoAgendada').text(`${interv.dataInterve} às ${interv.horaInterve}`);

  const dataFinal = interv.datafinal || 'Não finalizada';
  const horaFinal = interv.horafinal || '';
  $('#infoFinal').text(`${dataFinal}${horaFinal ? ' às ' + horaFinal : ''}`);

  const pecas = Array.isArray(interv.pecasUsadas) && interv.pecasUsadas.length > 0
    ? interv.pecasUsadas.join(', ')
    : 'Nenhuma';
  $('#infoPecas').text(pecas);

  $('#infoObs').text(interv.obsv || 'Sem observações');

  // Mostrar modal
  $('#infoIntervencaoModal').modal('show');
}
